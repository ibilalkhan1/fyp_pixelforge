// Package pixelforge_key provides functionality for a virtual keyboard.
//
...
//
// The pixelforge_key package supports up to 119 keys but defines only
...
// have even more than 119 keys, but Pixelforge does not support them.
...
func init() {
	onKey := func(event Event, _ pievent.Handler) {
		switch event.Type {
		case EventDown:
			keyState.SetDownFrame(event.Key, pixelforge.Frame)
		case EventUp:
			keyState.SetUpFrame(event.Key, pixelforge.Frame)
		}
	}
