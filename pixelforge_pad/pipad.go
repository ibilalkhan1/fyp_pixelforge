// Package pixelforge_pad provides functionality for a virtual gamepad
...
//     and on Switch it's "B". All of these map to the same constant pixelforge_pad.A,
...
//
// The pixelforge_pad package supports up to 16 buttons but defines only
...
// have even more than 16 buttons, but Pixelforge does not support them.
...
func onButton(event EventButton, _ pievent.Handler) {
	...
	switch event.Type {
	case EventDown:
		buttonState[event.Player].SetDownFrame(event.Button, pixelforge.Frame)
	case EventUp:
		buttonState[event.Player].SetUpFrame(event.Button, pixelforge.Frame)
	}
}
