...
func TestShapes(t *testing.T) {
	t.Run("should draw shapes in the same way as in shapes.png", func(t *testing.T) {
		pixelforge.ResetPaletteMapping()
		pixelforge.ResetColorTables()
		pixelforge.Palette = pixelforge.DecodePalette(shapes)
		shapesSheet := pixelforge.DecodeCanvas(shapes)
		...
		for testName, shape := range tests {
			t.Run(testName, func(t *testing.T) {
				pixelforge.SetScreenSize(32, 32)
				pixelforge.Cls()
...
func rect(x0, y0, x1, y1 int) func() {
	return func() {
		pixelforge.Rect(x0, y0, x1, y1)
	}
}

func rectfill(x0, y0, x1, y1 int) func() {
	return func() {
		pixelforge.RectFill(x0, y0, x1, y1)
	}
}

func circ(cx, cy, r int) func() {
	return func() {
		pixelforge.Circ(cx, cy, r)
	}
}

func circfill(cx, cy, r int) func() {
	return func() {
		pixelforge.CircFill(cx, cy, r)
	}
}
