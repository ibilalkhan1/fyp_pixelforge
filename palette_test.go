...
func TestDecodePaletteOrErr(t *testing.T) {
	t.Run("should return error when palette has too many colors", func(t *testing.T) {
		...
		for testName, file := range tests {
			t.Run(testName, func(t *testing.T) {
				palette, err := pixelforge.DecodePaletteOrErr(file)
	...
	t.Run("should not return error when palette has maximum number of colors", func(t *testing.T) {
		...
		for testName, file := range tests {
			t.Run(testName, func(t *testing.T) {
				_, err := pixelforge.DecodePaletteOrErr(file)
	...
	t.Run("should decode palette", func(t *testing.T) {
		expected := pixelforge.PaletteArray{0x000000, 0x1900ff, 0xff00b0, 0xff2600}
		tests := map[string][]byte{
...
func TestFromRGB(t *testing.T) {
	// when
	rgb := pixelforge.FromRGB(1, 128, 255)
	// then
	r, g, b := rgb.RGB()
...
func TestFromRGBf(t *testing.T) {
	// when
	rgb := pixelforge.FromRGBf(0.004, 0.5, 1.0)
	// then
	rf, gf, bf := rgb.RGBf()
...
func TestRGB_String(t *testing.T) {
	rgb := pixelforge.RGB(0x304050)
	assert.Equal(t, "0x304050", rgb.String())
}

func TestPaletteArray_String(t *testing.T) {
	var p pixelforge.PaletteArray
	p[0] = pixelforge.RGB(0x010101)
	p[1] = pixelforge.RGB(0x020202)
	p[63] = pixelforge.RGB(0x636363)
	// when
	actual := p.String()
	// then
	assert.Equal(t,
